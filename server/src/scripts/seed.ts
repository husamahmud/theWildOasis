...
const seed = async () => {
  ...
  for (let i = 0; i < bookings.length; i++) {
    const booking = await prisma.bookings.create({
      data: {
        ...bookings[i],
        startDate: new Date(bookings[i].startDate),
        endDate: new Date(bookings[i].endDate),
        status: bookings[i].status as Status,
        cabinId: cabinsIDs[i % cabinsIDs.length],
        guestId: guestsIDs[i % guestsIDs.length],
      },
    })
